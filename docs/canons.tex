...
\usepackage[canon=vdg, margins=symmetric, paper=letterpaper, showframe]{canons}
\usepackage{tikz}
\usepackage{float}
\usepackage{booktabs}
\usepackage{lipsum}
...
\title{\texttt{canons.sty}\\[0.5\baselineskip]
...
\pkg{canons} implements classical page-layout canons with in-house margin notes and sidenotes. Its one declared external dependency is \pkg{geometry}; all other functionality is self-contained. The package requires Lua\LaTeX.
...
\pkg{canons} offers four margin modes (\texttt{symmetric}, \texttt{antisymmetric}, \texttt{right}, \texttt{left}), gutter support with two calculation modes, a \texttt{fullwidth} environment, and exported dimensions for margin-aware content.

Use \pkg{canons} when you want canonical proportions and consistent marginalia defaults without adopting a full document class. It sits above \pkg{geometry}: \pkg{canons} handles the math; \pkg{geometry}, the implementation. For end-to-end book production, use \pkg{memoir} or KOMA-Script; for a curated editorial idiom, use \pkg{tufte-book}.
...
\pkg{canons} implements canons as parametric rules, with an optional \texttt{guttermode=satzspiegel} that switches the operative frame from page $P$ to leaf $L$ by recomputing on $W_\ast=W-g$. The Honnecourt family is encoded algebraically (a constructive origin; an algebraic implementation).

\section{Related work}\label{sec:related}

KOMA-Script \cite{koma-script} \pkg{typearea} chooses a typeblock via \texttt{DIV}, \texttt{BCOR}, and class-level heuristics sensitive to font details; \pkg{canons} fixes canonical ratios and shims the result through \pkg{geometry}. Use KOMA for class-integrated page design; use \pkg{canons} for literal canons.

\pkg{memoir} \cite{memoir} offers a comprehensive layout calculus (\cmd{settypeblocksize}, \cmd{setlrmargins}, diagnostics) and is a full publishing toolkit. For end-to-end book production, \pkg{memoir} is the right hammer; \pkg{canons} is a small wrench: canons, gutters, marginalia.

\pkg{tufte-book} \cite{tufte-latex} embodies a coherent editorial idiom beyond raw geometry: wide outer margins, sidenotes, specialized floats. \pkg{canons} can mimic the broad proportions but does not implement the idiom.
...
If you already know your exact margins, \pkg{geometry} alone is simpler; if you want canonical proportions and consistent marginalia defaults, \pkg{canons} saves you from re-deriving them.

\section{Limitations}\label{sec:limits}

\pkg{canons} deliberately restricts itself to page geometry:
\begin{itemize}
...
\part{Using canons}\label{part:using}
...
\begin{fullwidth}
	\begin{verbatim}
\usepackage[canon=grid, gridN=9, gridinner=1, gridouter=2, gridtop=1, gridbottom=2]{canons}
	\end{verbatim}
\end{fullwidth}
...
\usepackage[canon=grid, gridN=9, gridinner=1, gridouter=3]{canons}
\end{verbatim}
...
Loading \pkg{canons} in twocolumn mode emits a package warning; marginalia placement is not guaranteed. Not tested here.
